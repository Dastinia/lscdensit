...
//              containerInfoService.saveByResponse(dataRedisResponse, dataRedisDClient, aiUnitId, info.getModelServiceIp(), "dataRedis"+ aiUnitId, "数据流redis");
              // todo docker服务端口不能写死
              containerInfoService.saveByResponse(kafkaServiceResponse, kafkaDClient, aiUnitId, info.getKafkaServiceIp(), 7788,"kafkaService"+ aiUnitId, "卡夫卡抓取服务");
//              boolean moniterInfoSaved = containerInfoService.saveByResponse(moniterServiceResponse, kafkaDClient, aiUnitId, info.getmoniterServiceIp().substring(6,16),"moniterService-" + aiUnitId, "监控服务");
              containerInfoService.saveByResponse(modelServiceResponse, modelDClient, aiUnitId, info.getModelServiceIp(), 7789,"stationModelService" + aiUnitId, "车站预测模型服务");
              log.info("容器数据入库成功");
